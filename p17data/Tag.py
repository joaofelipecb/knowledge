...
versions['0.0.0.1.1']['list']['query'] = 'select tag_id, tag_name from tags order by tag_name'
...
where t1.tag_name in %s
and t2.tag_name not in %s
group by t2.tag_id
having count(distinct t1.tag_id) = %s
